...
def setting_bg():
    st.markdown(f""" <style>.stApp {{
                        background: url("https://wallpapers.com/images/hd/data-exploding-effect-1h87ygd4y3dws8qs.jpg");
                        background-size: cover}}
                     </style>""",unsafe_allow_html=True) 
setting_bg()

# CREATING OPTION MENU
selected = option_menu(None, ["Home","Upload & Extract","Edit"], 
                       icons=["house","cloud-upload","pencil-square"],
                       default_index=0,
                       orientation="horizontal",
                       styles={"nav-link": {"font-size": "35px", "text-align": "centre", "margin": "0px", "--hover-color": "#ba1496"},
                               "icon": {"font-size": "35px"},
                               "container" : {"max-width": "6000px"},
                               "nav-link-selected": {"background-color": "#ba1496"}})
...
if selected == "Home":
    ...
    with col1:
        ...
        st.markdown("## :green[**Overview :**] This Streamlit web app enables users to upload business card images, extract data using easyOCR, and manage the extracted information. Users can view, edit, or delete data, and save it in a database, allowing storage of multiple entries with associated images and extracted details.")
    with col2:
...
if selected == "Upload & Extract":
    ...
    if uploaded_card is not None:
        ...
        def save_card(uploaded_card):
            # Define the directory where you want to save the uploaded cards.
            ...
            # Save the uploaded card in the directory.
            with open(os.path.join(save_directory, uploaded_card.name), "wb") as f:
                f.write(uploaded_card.getbuffer())
        
        # Replace 'uploaded_card' with the actual object representing your uploaded card.
        # For example: save_card(your_uploaded_card_object)
